...
$result = generatePageLinks($pager, 2);
$pageLinks = $result["links"];
$prevLink=$result["prev"];
$nextLink=$result["next"];
?>

<div class="pager" aria-label="<?= lang('Pager.pageNavigation') ?>">
	<ul class="pagination">
			<li class="button p-sm-2">
        ...
        <?php foreach ($pageLinks as $link) : ?>
			<li class="<?= $link['active'] ? "pagination-link font-weight-bold p-sm-2" : 'pagination-link p-sm-2 '.@$link['class'] ?>">
				<a href="<?= $link['uri'] ?>" class="text-dark">
					<?= $link['title'] ?><span class="d-sm-none"> / <?= $pages ?></span>
				</a>
			</li>
		<?php endforeach ?>

			<li class="button p-sm-2" >
				<a href="<?= @$nextLink['uri'] ?>" <?php if (!$nextLink) echo "disabled" ?> aria-label="<?= lang('Pager.next') ?>">
					<span aria-hidden="true">
